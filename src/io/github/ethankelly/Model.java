...
public class Model {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Initialise graphs using random graph generator
        Graph erdosRenyi = GraphGenerator.erdosRenyi(numVertices, p);
        Graph tree = GraphGenerator.tree(numVertices);
        Graph grid = GraphGenerator.complete(numVertices);

        // Initialise the models
        Model m = new Model(numVertices, erdosRenyi);
        Model n = new Model(numVertices, erdosRenyi);
        Model o = new Model(numVertices, erdosRenyi);
    ...
    private void initialiseIdenticalModel(int outbreak, Model original) {
        for (int i = 0; i < original.getNumVertices(); i++) {
            this.getAgents().set(i, new Agent(i, 0, 0, State.SUSCEPTIBLE));
            this.getAgents().get(i).setPeril(
                    original.perilRating(this.getAgents().get(i), new int[]{outbreak}, this.getGraph()));
            this.getAgents().get(i).setProtection(
                    original.protectionRating(this.getAgents().get(i)));
            this.getAgents().get(i).setState(
                    this.findState(this.getAgents().get(i), new int[]{outbreak}));
